...
@torch.no_grad()
def diffusion_step(model, latents, context, t, guidance_scale, control_pose=None, low_resource=False):
    control_pose = prepare_image(
                image=control_pose,
                width=512,
                height=512,
                batch_size=1 * 1,
    ...
    if low_resource:
        ...
        down_block_res_samples, mid_block_res_sample, means, stds= model.controlnet(
                    controlnet_latent_model_input.to(model.controlnet.dtype),
                    t.to(model.controlnet.dtype),
                    encoder_hidden_states=controlnet_prompt_embeds.to(model.controlnet.dtype),
                    controlnet_cond=control_pose.to(model.controlnet.dtype),
                    conditioning_scale=controlnet_conditioning_scale,
                    guess_mode=guess_mode,
                    return_dict=False,
                )
        # for _ in down_block_res_samples:
        #     print(_.shape)
        means1=torch.cat(down_block_res_samples[:3]).mean()
       

        stds1=torch.cat(down_block_res_samples[:3]).std()
...
def text2image_ldm_stable(
    ...
    for t in tqdm(model.scheduler.timesteps):
        latents, mean, std = diffusion_step(model, latents, context, t, guidance_scale, control_pose=control_pose, low_resource = low_resource)
        means.append(mean)
        stds.append(std)
    image = latent2image(model.vae, latents)
  
    return image, latent, means, stds
